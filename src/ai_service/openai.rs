...
不要輸出其他欄位，也不要添加額外文字。"#,
        ...
        if let Ok(body) = serde_json::to_string(&primary_request) {
            log::info!(
                "[AI INPUT][generate_task_with_expert_primary][OpenAI] {}",
        ...
        let primary_text = primary_response.text().await?;
        log::info!(
            "[AI OUTPUT][generate_task_with_expert_primary][OpenAI] {}",
            format_ai_output(&primary_text)
        );

        if !primary_status.is_success() {
            return Err(anyhow::anyhow!(
                "OpenAI API 錯誤 (primary) ({}): {}",
                primary_status,
                primary_text
            ));
        }
...
不要修改主要欄位，不要輸出額外文字。"#,
        ...
        if let Ok(body) = serde_json::to_string(&secondary_request) {
            log::info!(
                "[AI INPUT][generate_task_with_expert_secondary][OpenAI] {}",
                format_ai_output(&body)
            );
        }
        ...
        let secondary_text = secondary_response.text().await?;
        log::info!(
            "[AI OUTPUT][generate_task_with_expert_secondary][OpenAI] {}",
            format_ai_output(&secondary_text)
        );

        if !secondary_status.is_success() {
            return Err(anyhow::anyhow!(
                "OpenAI API 錯誤 (secondary) ({}): {}",
                secondary_status,
                secondary_text
            ));
        }
