...
pub struct Task {
    ...
    pub task_category: Option<String>, // 任務分類：'main', 'daily', 'project', 'milestone'
}
crud!(Task{});
    ...
    match value {
        serde_json::Value::Number(n) => {
            if let Some(i) = n.as_i64() {
                ...
                Ok(f.round() as i32) // 浮點數四捨五入轉整數
            } else {
                Err(D::Error::custom("無法轉換為整數"))
            }
        }
        serde_json::Value::String(s) => {
            s.parse::<i32>().map_err(|_| D::Error::custom("字符串無法解析為整數"))
        }
        _ => Err(D::Error::custom("期望數字類型")),
    }
} 